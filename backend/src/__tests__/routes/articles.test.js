...
describe('Testing all /articles endpoints', () => {
  ...
  describe('Retrieve articles', () => {
    ...
    it('should return all articles', async () => {
      ...
      expect(data.articles.length).toEqual(2);
    ...
    it('should return a specific article', async () => {
      ...
      const testArticleSlug = data.articles[0].slug;

      const response = await requests.get(`${articlesUrl}/${testArticleSlug}`);

      expect(response.data.article.slug).toEqual(testArticleSlug);
      expect(response.data.article.title).toEqual(articles[1].title);
  ...
  describe('Post, update and delete article', () => {
    ...
    it('should post article successfully', async () => {
      ...
      expect(data.article.title).toEqual(articles[0].title);
    });

    it('should update article successfully', async () => {
      const token = await getToken();
      const articlePosted = await requests.post(
        '/articles',
        articleBody,
        token
      );
      const articleSlug = articlePosted.data.article.slug;
      ...
      expect(data.article.title).toEqual(updatedArticle.article.title);
    });
